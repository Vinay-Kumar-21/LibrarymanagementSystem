...
router.post("/borrow",async(req,res,next)=>{
    try{
        const books=await bookModel.findOne({isbn:req.body.isbn});
        if(books==null){
            return res.status(400).json({error:"book not found"});
        }
        if(books.borrowedBy===books.quantity){
            return res.status(400).json({error:"Book is not available"});
        }

        const user=await userModel.findById(rq.body.userId)
        if(user==null){
            return res.status(404).json({error:"user not found"});
        }
        if(books.borrowedBy.includes(user.id)){
            return res.status(400).json({error:"you've already borrowed this book"});
        }

        await books.update({borrowedBy:[...books.borrowedBy,user.id]});
        const updatedBook=await bookModel.findById(books.id);
...
router.post("/return",async(req,res,next)=>{
    try{
        ...
        console.log("filtered",book.borrowedBy.filter((borrowedBy)=>!borrowedBy.equals(user.id)));
